...
@section('content')
<div class="container">
    <div class="row" id="table_details"></div>
            ...
            <nav>
                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                    @foreach($categories as $category)
                        <a class="nav-item nav-link" data-id="{{$category->id}}" data-toggle="tab" href="">{{$category->name}}</a>
                    @endforeach
                </div>
            </nav>
...
<script>
    $(document).ready(function() {
        ...
        // make table_details hidden by default
        $('#table_details').hide();

        // show all of the tables when the client clicks the button
        $('#btn-show-tables').click(function() {
            if ( $('#table_details').is(":hidden") ) {
                $.get("/cashier/getTable", function(data) {
                    $("#table_details").html(data);
                    $("#table_details").slideDown('fast');
                    $('#btn-show-tables').html('Hide Tables').removeClass('btn-primary').addClass('btn-danger');
                })    
            } else {
                $("#table_details").slideUp('fast');
        ...
        $("#table_details").on("click", ".btn-table", function (){
    ...
    // button click for order details
    ...
    // delete sale details from table
    ...
    $("#recieved-amount").keyup(function() {
        ...
        // check if cashier entered the right amount, then enable/disable save payment button
        if ( changeAmount >= 0 ) {
